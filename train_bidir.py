...
from keras.layers import Dense, Embedding, LSTM, Dropout, Flatten, TimeDistributed, BatchNormalization, Bidirectional
...
if new_model:
    ...
    model = Sequential()
    model.add(Bidirectional(LSTM(lstm_out, dropout = 0.2, return_sequences = True), input_shape = X_train.shape[1:]))
    model.add(Flatten())
