...
from keras.layers import Dense, Embedding, LSTM, Dropout, Flatten, TimeDistributed, BatchNormalization
...
if new_model:
    ...
    model = Sequential()
    model.add(LSTM(lstm_out, input_shape = X_train.shape[1:], dropout = 0.2, return_sequences = True))
    model.add(Flatten())
    ...
    print('\rLoaded lstm model from disk')
...
for i in range(cy):
    ...
    for j in range(reset_train):
        ...
        # Save
        print('Saving lstm model...', end = '')
        model_json = model.to_json()
        with open(model_name + '.json', 'w') as json_file:
            json_file.write(model_json)
        model.save_weights(model_name + '.h5')
        print('\rSaved lstm model to disk')
